...
- (void)layoutSubviews
{
    ...
    [self.leftIcon mas_makeConstraints:^(MASConstraintMaker * make) {
        ...
        make.left.equalTo(weakSelf).offset(CURRENT_SIZE(0));
    }];
    ...
    [self.textField mas_makeConstraints:^(MASConstraintMaker * make) {
        ...
        make.centerY.equalTo(weakSelf);
        make.left.equalTo(weakSelf.leftIcon.mas_right).offset(CURRENT_SIZE(0));
    }];
    ...
    [self.lengthLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.size.mas_equalTo(CGSizeMake(CURRENT_SIZE(0), CURRENT_SIZE(15)));
    ...
    [self.bottomLine mas_makeConstraints:^(MASConstraintMaker *make) {
        make.size.mas_equalTo(CGSizeMake(SCW-CURRENT_SIZE(10), 1));
        make.left.equalTo(weakSelf).offset(10);
...
- (UIButton *)hideBtn{
    if (!_hideBtn) {
        ...
        [_hideBtn setImage:[UIImage imageNamed:@""] forState:UIControlStateNormal];
        [_hideBtn setImage:[UIImage imageNamed:@""] forState:UIControlStateSelected];
        [_hideBtn addTarget:self action:@selector(hideAction:) forControlEvents:UIControlEventTouchUpInside];
    }
    return _hideBtn;
}

- (void)hideAction:(UIButton *)hideBtn{
