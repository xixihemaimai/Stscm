...
/**
 默认滚动到当前时间
 */
-(instancetype)initWithDateStyle:(WSDateStyle)datePickerStyle CompleteBlock:(void(^)(NSDate * selectDate))completeBlock {
    self = [super init];
    if (self) {
...
/**
 滚动到指定的的日期
 */
-(instancetype)initWithDateStyle:(WSDateStyle)datePickerStyle scrollToDate:(NSDate *)scrollToDate CompleteBlock:(void(^)(NSDate * selectDate))completeBlock {
    self = [super init];
    if (self) {
        self = [[[NSBundle mainBundle] loadNibNamed:NSStringFromClass([self class]) owner:self options:nil] lastObject];
...
-(void)setupUI {
    ...
    self.doneButtonColor = RGB(252, 200, 40);
...
-(void)addLabelWithName:(NSArray *)nameArr {
    ...
    if (!_dateLabelColor) {
        _dateLabelColor =  RGB(252, 200, 40);
    }
...
- (IBAction)doneAction:(UIButton *)btn {
    ...
    //[btn setBackgroundColor:[UIColor colorWithHexColorStr:@"#27c79a"]];
